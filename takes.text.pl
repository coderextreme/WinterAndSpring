...
open(SCENE, "<WinterAndSpringHumanoidTimingsRoot0406.txt") or die "Couldn't open main stage\n";
...
for ($c = 0; $c < @chars; $c++) {
	print "\n$chars[$c]\t\t";
	print join(" ", @{$timesperchar{$chars[$c]}})."\n";
	print "\t\t".join(" ", @{$movesperchar{$chars[$c]}})."\n";
	# print join(" ", @{$char{$chars[$c]}->{moves}})."\n";
	my $movepercharprev = "";
	my $movepercharnext = "";
