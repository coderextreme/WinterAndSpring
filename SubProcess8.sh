...
#cat "${TEMPLATE}" | sed -e "s/IMAGE.png/${IMAGE}/" > "${JOINTOUTPUT}"
#exit_on_failure $? "Couldn't cat ${TEMPLATE} | sed -e s/IMAGE.png/${IMAGE}/ > ${JOINTOUTPUT}"
...
# perl haveSkeletonAddBoxes.pl "${WEIGHTS}" < "${CENTEROUTPUT}" > "${BOXOUTPUT}"
# exit_on_failure $? "Couldn't create ${BOXOUTPUT} using perl haveSkeletonAddBoxes.pl ${WEIGHTS} < ${CENTEROUTPUT} > ${BOXOUTPUT}"

#cp "${CENTEROUTPUT}" "${REVISEDOUTPUT}"
#exit_on_failure $? "Couldn't copy from $MIDCENTEROUTPUT to ${REVISEDOUTPUT}"
perl haveSkeletonAddSkinCoord.pl "${WEIGHTS}" <  "${CENTEROUTPUT}" > "${REVISEDOUTPUT}"
exit_on_failure $? "Couldn't create ${REVISEDOUTPUT} using perl haveSkeletonAddSkinCoord.pl ${WEIGHTS} < $CENTEROUTPUT > ${REVISEDOUTPUT}"
...
#awk 'BEGIN {for (j = 0; j < 1; j += 1/sqrt(20889)) for (i=1; i >= 0; i -= 1/sqrt(20889)) print i, j; }' > "${PROCESSDIR}/texturenos.txt"
#exit_on_failure $? "Couldn't create a list of number from 0 to 1 to place in ${PROCESSDIR}/texturenos.txt"
...
exit_on_failure $? "Couldn't create ${TAKES} usd takes.js < ${TAKESJSON}"
cat "${TIMERS}" >> "${FINAL}"
...
echo "cat $WEIGHTS | sed -n 's/.*(\(.*\)).*/\1/p' | sort -nu > $WEIGHTSOUTPUT"
...
#
echo "awk 'BEGIN {for (i=0; i < '${numCoords}'; i++) print i}' > $SKELETONOUTPUT"
...
echo "did diff -s $SKELETONOUTPUT $WEIGHTSOUTPUT | grep -v , | sed 's/^<.//'"
...
exit_on_failure $? "Couldn't get counts ${FINAL}" 1 "Couldn't find counts"
