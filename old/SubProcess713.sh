...
cat "${METATEMPLATE}" | perl -p -e s/Toddler/"${CHARACTER}"/g > "$TEMPLATE"
exit_on_failure $? "Problems substituting $CHARACTER ${METATEMPLATE} to $TEMPLATE"

unix2dos "$TEMPLATE"
exit_on_failure $? "install unix2dos"

perl replacejoints.pl < "$TEMPLATE" > "$JOINTOUTPUT"
exit_on_failure $? "Couldn't create ${JOINTOUTPUT} using perl replacejoints.pl < ${TEMPLATE} > ${JOINTOUTPUT}"

# minus center.pl program
#perl SkeletonParseLine.pl "$JOINTOUTPUT" "${WEIGHTS}" "${CENTERS}" | sed -e "s/IMAGE.png/${IMAGE}/" > "${OUTPUT}"
#exit_on_failure $? "Couldn't create ${OUTPUT} using perl SkeletonParseLine.pl ${JOINTOUTPUT} ${WEIGHTS} ${CENTERS}"
#
perl centers.pl "${CENTERS}" < "$JOINTOUTPUT" > "$CENTEROUTPUT"
...
# echo Now append "${ANIMATIONS}" to $REVISEDOUTPUT to add animation
# this doesn't work for final production
# cat "$ANIMATIONS" >> "$REVISEDOUTPUT" 

#perl centers.pl "${CENTERS}" < "$JOINTOUTPUT" > "$CENTEROUTPUT"
#exit_on_failure $? "Couldn't create ${CENTEROUTPUT} using perl centers.pl < ${JOINTOUTPUT} > ${CENTEROUTPUT}"
#perl SkeletonParseLine.pl "$JOINTOUTPUT" "${WEIGHTS}" "${CENTERS}" | sed -e "s/IMAGE.png/${IMAGE}/" > "$REVISEDOUTPUT"
#exit_on_failure $? "Couldn't create ${REVISEDOUTPUT} using perl SkeletonParseLine.pl ${JOINTOUTPUT} ${WEIGHTS} ${CENTERS}"
